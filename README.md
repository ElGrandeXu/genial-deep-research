...
Un dossier peut conserver jusqu’à douze faits métier uniques et vise huit à douze faits lorsque les preuves existent, avec au moins deux catégories, deux pages et deux familles d’éditeurs pour être complet. Un ou deux faits métier valides restent un résultat limité. Les faits d’une organisation explicitement liée restent sur un sujet distinct, dans « Contexte de l’organisation » ; ils ne complètent pas artificiellement le dossier personnel. Une homonymie non résolue produit une clarification ; l’absence de preuve produit un silence explicite ; une erreur technique ne produit aucun dossier partiel.
...
- Le mode explicite `waf_only` confie la cadence distribuée au WAF global Vercel : vingt `POST /api/research` par IP et par fenêtre fixe de dix minutes, avec action `rate_limit` et réponse `429`. Le WAF compte toute requête atteignant la route, y compris une requête invalide ; l’application valide néanmoins toujours le corps avant de construire le fournisseur, donc une entrée invalide ne génère aucun coût fournisseur. La mémoire de l’instance ne gère que la concurrence, limitée à deux dossiers simultanés.
