...
export const PROVIDER_INSTRUCTIONS = [
    ...
    "Effectue normalement au moins deux requêtes Web Search distinctes : commence par une recherche du nom exact entre guillemets, puis complète avec les indices utiles du contexte ou avec une organisation, un secteur, une ville ou une source d’identité détectée. Ne t’arrête après une seule requête que si aucun signal public n’existe même avec une variation complémentaire. Pour une entreprise, complète avec la ville, le secteur, les dirigeants, l’activité, les chiffres ou l’actualité. Reste dans un petit nombre borné de requêtes utiles.",
